...
const IconBadge = ({ badgeFill, badgeText }) => {
  ...
  return (
    <svg className={styles['icon-badge']} viewBox="0 0 40 40">
      <circle
      ...
      />
      <text
        className={cx(styles['badge-text'], {
...
IconBadge.asString = (badgeFill, badgeText) => {
  ...
  return `
  <svg class="${styles['icon-badge']}" viewBox="0 0 40 40">
    <circle class="badge-circle" cx="20" cy="20" fill="${badgeFill}" r="20"/>
    <text class="${cx(styles['badge-text'], {
      long: isBadgeTextLong(badgeText),
...
function Icon({
  ...
  return (
    <span aria-hidden className={styles['icon-container']} style={style}>
      <svg
        id={id}
        style={{
          fill: color || null,
...
Icon.asString = ({
  ...
  <span class="${styles['icon-container']}">
    <svg
      ${id ? ` id=${id}` : ''}
      viewBox="0 0 40 40"
      class="${cx(styles.icon, className)}"
    >
      ${
        backgroundShape === 'circle'
          ? `<circle className="${
              styles['icon-circle']
            }" cx="20" cy="20" fill="white" r="20" />`
          : ''
      }
